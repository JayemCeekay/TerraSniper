...
public class RandomErodeBrush extends AbstractBrush {
    ...
    private void randomErosion(Snipe snipe) throws MaxChangedBlocksException {
                        ...
                        for (int y = 1; y < this.snap.length - 1; y++) {
                            if (((xSquared + Math.pow(y - (this.brushSize + 1), 2) + zSquared) <= brushSizeSquared)) {
                                if (this.erode(x, y, z)) {
                                    ...
                                    setBlock(block.getX(), block.getY(), block.getZ(), BlockTypes.AIR.getDefaultState());
                                }
                            }
                        }
                    }
                    ...
                    for (int x = 1; x < this.snap.length - 1; x++) {
                        ...
                        for (int y = 1; y < this.snap.length - 1; y++) {
                            if (((xSquared + Math.pow(y - (this.brushSize + 1), 2) + zSquared) <= brushSizeSquared)) {
                                if (this.fill(x, y, z)) {
                                    BlockWrapper block = this.snap[x][y][z];
                                    setBlock(block.getX(), block.getY(), block.getZ(), this.snap[x][y][z].getType().getDefaultState());
                                }
                            }
                        }
                    }
    ...
    private void randomFilling(Snipe snipe) throws MaxChangedBlocksException {
        if (this.fillFace >= 0 && this.fillFace <= 6) {
