...
public class Sniper {
    ...
    public boolean snipe(ServerPlayer player, InteractionHand action, ItemStack usedItem, @Nullable BlockVector3 clickedBlock) {
        if (this.toolkits.isEmpty()) {
            ...
            if (toolkit == null) {
                player.sendSystemMessage(MutableComponent.create(new LiteralContents(TextColor.fromLegacyFormat(ChatFormatting.RED) + "No toolkit found for this item.")));
                return false;
            } else {
                ToolAction toolAction = toolkit.getToolAction(usedItem);
                if (toolAction == null) {
                    player.sendSystemMessage(MutableComponent.create(new LiteralContents(TextColor.fromLegacyFormat(ChatFormatting.RED) + "No tool action found for this item.")));
                    return false;
                } else {
            ...
            try {
                ...
                if (action != null) {
                    if (targetBlock.getY() < editSession.getWorld().getMinY() && Materials.isEmpty(editSession.getBlock(BlockVector3.at(targetBlock.getX(), targetBlock.getY(), targetBlock.getZ())).getBlockType())) {
                        player.sendSystemMessage(MutableComponent.create(new LiteralContents(TextColor.fromLegacyFormat(ChatFormatting.RED) + "Snipe target block must be visible.")));
                        return;
                    }
    ...
    public void sendInfo(Player sender) {
        ...
        if (toolkit == null) {
            sender.sendSystemMessage(MutableComponent.create(new LiteralContents("Current toolkit: none")));
        } else {
            sender.sendSystemMessage(MutableComponent.create(new LiteralContents("Current toolkit: " + toolkit.getToolkitName())));
            BrushProperties brushProperties = toolkit.getCurrentBrushProperties();
            Brush brush = toolkit.getCurrentBrush();
            if (brush == null) {
                sender.sendSystemMessage(MutableComponent.create(new LiteralContents("No brush selected.")));
            } else {
                ToolkitProperties toolkitProperties = toolkit.getProperties();
    ...
    public void updateItemStackInfo(Toolkit toolkit) {
        toolkit.getToolActions().forEach((toolMaterial, action) -> {
            ...
            if (this.getPlayer().getInventory().contains(toolMaterial)) {
                ...
                toolMaterial.setHoverName(MutableComponent.create(new LiteralContents(ChatFormatting.AQUA + "" + ChatFormatting.BOLD + "" + ChatFormatting.UNDERLINE + "" + toolkit.getCurrentBrushProperties().getName() + "-" + ChatFormatting.ITALIC + action.toString())));
                CompoundTag display = toolMaterial.getOrCreateTagElement("display");
                ListTag tag = display.getList("Lore", 8);
                tag.clear();
                tag.add(StringTag.valueOf(MutableComponent.Serializer.toJson(MutableComponent.create(new LiteralContents(ChatFormatting.DARK_AQUA + "Size: " + toolkit.getProperties().getBrushSize())))));
                toolkit.getCurrentBrush().getSettings().forEach((setting, value) -> {
                    tag.add(StringTag.valueOf(MutableComponent.Serializer.toJson(MutableComponent.create(new LiteralContents(ChatFormatting.DARK_AQUA + setting + ": " + value)))));
                });
                tag.add(StringTag.valueOf(MutableComponent.Serializer.toJson(MutableComponent.create(new LiteralContents(ChatFormatting.BLUE + "Material: " + toolkit.getProperties().getPattern().asBlockState())))));
                tag.add(StringTag.valueOf(MutableComponent.Serializer.toJson(MutableComponent.create(new LiteralContents(ChatFormatting.BLUE + "Replacing: " + toolkit.getProperties().getReplacePattern().toString())))));
    ...
    public void cleanToolkits() {
        this.toolkits.forEach(toolkit -> {
            if (toolkit.getToolActions().size() == 0) {
                ...
                this.player.sendSystemMessage(MutableComponent.create(new LiteralContents("Removed toolkit " + toolkit.getToolkitName() + " because it has no actions.")));
            }
        });
    }
}
