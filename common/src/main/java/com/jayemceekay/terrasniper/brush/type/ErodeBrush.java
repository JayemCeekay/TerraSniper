...
public class ErodeBrush extends AbstractBrush {
    ...
    private static final Map<String, Boolean> IS_ADDITIVE = Map.ofEntries(
            ...
            Map.entry("melt", false),
            Map.entry("fill", true),
            Map.entry("smooth", true),
            Map.entry("lift", true),
            Map.entry("floatclean", false)
            );
    ...
    public void handleCommand(String[] parameters, Snipe snipe) {
        ...
        for (int var6 = 0; var6 < var5; ++var6) {
            ...
            if (IS_ADDITIVE.containsKey(parameter)) {this.setAdditiveBrush(IS_ADDITIVE.get(parameter));}
    ...
    public void handleArrowAction(Snipe snipe) {
        this.erosion(snipe, this.currentPreset, (this.getAdditiveBrush() ? 1 : 0));
    }

    public void handleGunpowderAction(Snipe snipe) {
        this.erosion(snipe, this.currentPreset.getInverted(), (this.getAdditiveBrush() ? 0 : 1));
    }

    private void erosion(Snipe snipe, ErosionPreset erosionPreset, int brushSizeChange) {
        ToolkitProperties toolkitProperties = snipe.getToolkitProperties();
        BlockVector3 targetBlock = this.getTargetBlock();
        BlockChangeTracker blockChangeTracker = new BlockChangeTracker(this);

        int i;
        for (i = 0; i < erosionPreset.getErosionRecursion(); ++i) {
            this.erosionIteration(toolkitProperties, erosionPreset, blockChangeTracker, targetBlock, brushSizeChange);
        }

        for (i = 0; i < erosionPreset.getFillRecursion(); ++i) {
            this.fillIteration(toolkitProperties, erosionPreset, blockChangeTracker, targetBlock, brushSizeChange);
        }
    ...
    private void fillIteration(ToolkitProperties toolkitProperties, ErosionPreset erosionPreset, BlockChangeTracker blockChangeTracker, BlockVector3 targetBlockVector, int brushSizeChange) {
        int currentIteration = blockChangeTracker.nextIteration();
        BlockVector3 targetBlock = this.getTargetBlock();
        int brushSize = toolkitProperties.getBrushSize() + brushSizeChange;
    ...
    private void erosionIteration(ToolkitProperties toolkitProperties, ErosionPreset erosionPreset, BlockChangeTracker blockChangeTracker, BlockVector3 targetBlockVector, int brushSizeChange) {
        int currentIteration = blockChangeTracker.nextIteration();
        BlockVector3 targetBlock = this.getTargetBlock();
        int brushSize = toolkitProperties.getBrushSize() + brushSizeChange;
    ...
    static class BlockChangeTracker {
        ...
        private final ErodeBrush brushReference;
        private int nextIterationId;

        private BlockChangeTracker(ErodeBrush brushReference) {
        ...
        public BlockWrapper get(BlockVector3 position, int iteration) {
            ...
            return new BlockWrapper(position.getBlockX(), position.getBlockY(), position.getBlockZ(), this.brushReference.getBlock(position));
        }
