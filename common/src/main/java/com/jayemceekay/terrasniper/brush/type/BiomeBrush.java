    ...
    private void biome(Snipe snipe) {
        ...
        for (int x = lowChunkX; x <= highChunkX; x++) {
            for (int z = lowChunkZ; z <= highChunkZ; z++) {
                ...
                snipe.getSniper().getPlayer().getServer().getPlayerList().getPlayer(snipe.getSniper().getPlayer().getUUID()).connection.send(new ClientboundLevelChunkWithLightPacket(snipe.getSniper().getPlayer().level().getChunk(x, z), snipe.getSniper().getPlayer().level().getLightEngine(), null, null));
            }
        }
    }
