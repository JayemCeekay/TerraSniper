        ...
        dispatcher.register(Commands.literal("ts")
                ...
                .then((Commands.literal("d").requires((commandSource) -> {
                    ...
                    if (sniper != null && sniper.getCurrentToolkit() != null) {
                        ...
                        sniper.updateItemStackInfo(sniper.getCurrentToolkit());
                        context.getSource().sendSystemMessage(Component.literal(ChatFormatting.AQUA + "Brush settings reset to their default values."));
                    } else {
                        (new MessageSender(sniper.getPlayer())).message(ChatFormatting.RED + "You must have a toolkit selected!").send();
                    }
                ...
                .then(Commands.literal("smallblocks").executes((context) -> {
                    Sniper sniper = TerraSniper.sniperRegistry.getSniper(context.getSource().getPlayerOrException().getUUID());
                    Messenger messenger = new Messenger(sniper.getPlayer());
                    if (sniper.smallBlocksEnabled()) {
                        sniper.setSmallBlocksEnabled(false);
                        messenger.sendMessage(ChatFormatting.RED + "SmallBlocks mode is now disabled.");
                    } else if (!sniper.smallBlocksEnabled()) {
                        sniper.setSmallBlocksEnabled(true);
                        messenger.sendMessage(ChatFormatting.GREEN + "SmallBlocks mode is now enabled.");
                    }
                    return 0;
                }))
