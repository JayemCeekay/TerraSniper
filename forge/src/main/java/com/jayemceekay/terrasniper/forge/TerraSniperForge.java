...
public class TerraSniperForge {
    ...
    public void onPlayerJoin(net.minecraftforge.event.entity.player.PlayerEvent.PlayerLoggedInEvent event) {
        TerraSniperEventHandler.onPlayerJoinEvent(event.getEntity().getServer().getPlayerList().getPlayer(event.getEntity().getUUID()));
    }

    @SubscribeEvent
    public void onPlayerLeave(net.minecraftforge.event.entity.player.PlayerEvent.PlayerLoggedOutEvent event) {
        TerraSniperEventHandler.onPlayerLeaveEvent(event.getEntity().getServer().getPlayerList().getPlayer(event.getEntity().getUUID()));
    }

    @SubscribeEvent
    public void onPlayerUse(PlayerInteractEvent.RightClickItem event) {
        TerraSniperEventHandler.onToolRightClick(event.getEntity(), event.getHand());
    }
